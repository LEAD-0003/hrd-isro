...
use App\Filament\Admin\Resources\TrainingApplicationResource\Pages;
        ...
        return $table
            ...
            ->actions([
                ...
                Tables\Actions\Action::make('approve')
                    ...
                    ->color('success')
                    ->form([
                        Forms\Components\Textarea::make('hq_comments')
                    ...
                    ->action(function ($record, $data) {
                        ...
                        $record->update([
                            'status' => 'approved',
                            'hq_comments' => $data['hq_comments']
                        ]);

                        if ($record->user?->email) {
                            Mail::to($record->user->email)
                                ->send(new TrainingStatusMail($record, 'approved'));
                        }

                        Notification::make()
                            ->title('Application Approved & Mail Sent')
                            ->success()
                            ->send();
                    }),
                ...
                Tables\Actions\Action::make('reject')
                    ...
                    ->action(function ($record, $data) {
                        ...
                        if ($record->user?->email) {
                            Mail::to($record->user->email)
                                ->send(new TrainingStatusMail($record, 'rejected'));
                        }

                        Notification::make()
                            ->title('Application Rejected & Mail Sent')
                            ->danger()
                            ->send();
                    }),
                ...
                Tables\Actions\Action::make('complete')
                    ...
                    ->action(function ($record) {
                        ...
                        if ($record->user?->email) {
                            Mail::to($record->user->email)
                                ->send(new TrainingStatusMail($record, 'completed'));
                        }
